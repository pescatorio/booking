...
function modifyBtn(url) {
	let tmpName = prompt("作成者名をご記入ください");
	if(tmpName!=""&&tmpName!=null){
		let tmpPasswd = prompt("パスワードをご記入ください");
		if(tmpPasswd!=""&&tmpPasswd!=null){
			$('#cName').val(tmpName);
			$('#cPasswd').val(tmpPasswd);
			document.detailForm.action = url;/* + "?name="+tmpName+"&passwd="+ tmpPasswd +"&no=${vo.no}&pageNo=${criteria.pageNo}&keyword=${criteria.keyword}"; */ 
			document.detailForm.submit();
		}else{
			...
			<div class="default_cell">
				<div class="table_data">
					...
					<form method="get" name="detailForm" id="modifyNotice">
						...
						<input type="hidden" id="cName" name="name" />
						<input type="hidden" id="cPasswd" name="passwd" />
